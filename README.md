...
![Home Assistant Minimum Version](https://img.shields.io/badge/Home%20Assistant-2023.8.0%2B-blue.svg?style=for-the-badge)
...
- 🚨 **Smart Alarms**: Receive binary sensor alerts for base alarms, raid detectors, or tripwires.
- 📦 **Storage Monitors**: Monitor inventory levels of chests and storage boxes (e.g., amount of sulfur, metal, or wood).
- 🏰 **Tool Cupboards**: View upkeep time remaining and quantity of materials inside your TC.
...
- 👥 **Team Tracking**: (Under the hood) integration structures for monitoring team status.
...
2. Copy the `custom_components/rustplus` folder to your Home Assistant's `config/custom_components/` directory.
...
4. Follow the setup prompts:
   - **FCM Credentials**: Provide the FCM credentials JSON string (used for receiving push notifications from Rust+).
   - **Server details**: Enter your server IP, port, Steam ID, and Player Token.
...
3. Go to the **Rust+ Assistant** integration page, click **Configure / Options**, and add the paired device using the format:
   `ENTITY_ID:Friendly Name` (separated by commas).
